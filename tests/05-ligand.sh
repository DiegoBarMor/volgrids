...
echo
echo ">>> TEST 05: Ligands"

folder=data/tests/05-ligand
...
python3 smiffer.py pack -i "${paths_grids[@]}" -o "$folder/phenol.cmap"  # pack them into a single CMAP file
...
python3 smiffer.py pack -i "${paths_grids[@]}" -o "$folder/1lph.cmap"
rm -f "${paths_grids[@]}"
