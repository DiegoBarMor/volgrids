...
class CavityFinder:
    ...
    def populate_cavities_grid(self, occupancy: vg.Grid):
        func: callable = self._find_cavities_naive_multi_pass \
            if sm.CAV_NPASSES > 1 else self._find_cavities_naive
        self.grid = func(occupancy)


    # --------------------------------------------------------------------------
    def apply_cavities_weighting(self, g: "vg.Grid"):
        if sm.CAV_WEIGHT == 0.0: return
        ...
        g.arr *= (1 + self.grid.arr * sm.CAV_WEIGHT)
    ...
    def _find_cavities_naive_multi_pass(occupancy: vg.Grid) -> vg.Grid:
        ...
        cavities_rot = (
            get_cavities_rot(i*90 / sm.CAV_NPASSES)
            for i in range(1, sm.CAV_NPASSES)
        )

        cavities_orig.arr = (cavities_orig.arr + sum(cav.arr for cav in cavities_rot)) / sm.CAV_NPASSES
